...
export const makeMessagesSocket = config => {
	...
	const sendReceipts = async (keys, type) => {
		...
		await Promise.allSettled(recps.map(({ jid, participant, messageIds }) => sendReceipt(jid, participant, messageIds, type)))
	}
	...
	const assertSessions = async (jids, force) => {
		...
		const validations = force
			? uniqueJids.map(() => ({ exists: false }))
			: await Promise.all(uniqueJids.map(jid => signalRepository.validateSession(jid)))
		const jidsRequiringFetch = uniqueJids.filter((_, i) => force || !validations[i].exists)
			...
			if (
				...
				if (config.emitOwnEvents) {
					process.nextTick(async () => {
						const mutexKey = fullMsg.key.participant || fullMsg.key.remoteJid
