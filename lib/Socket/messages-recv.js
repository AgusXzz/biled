...
export const makeMessagesRecvSocket = config => {
	...
	const identityAssertDebounce = new NodeCache({ stdTTL: 5, maxSize: 500, useClones: false })
	...
	const sendMessagesAgain = async (key, ids, retryNode, receiptNode) => {
		...
		const msgId = ids[0]
		// Fetch all messages concurrently — one failure doesn't kill the batch
		...
		const msgs = results.map((r, i) => {
			if (r.status === 'rejected') {
				logger.warn({ id: ids[i], err: r.reason }, 'failed to fetch message for retry')
				return undefined
			}
			return r.value
		})
		...
		const relayResults = await Promise.allSettled(
			msgs.map(async (msg, i) => {
				...
				if (msg && (await willSendMessageAgain(ids[i], participant))) {
					...
					return relayMessage(key.remoteJid, msg, msgRelayOpts)
				} else {
		...
		for (const r of relayResults) {
			if (r.status === 'rejected') {
				logger.warn({ err: r.reason }, 'failed to relay retry message')
			}
		}
	}
	const handleReceipt = async node => {
		...
		try {
			await receiptMutex.mutex(remoteJid, async () => {
	...
	const handleNotification = async node => {
		...
		try {
			await notificationMutex.mutex(remoteJid, async () => {
	...
	const handleMessage = async node => {
		...
		try {
			...
			const mutexKey = msg.key.participant || msg.key.remoteJid
	...
	ws.on('CB:call', node => {
		processNode('call', node, 'handling call', handleCall)
			.catch(error => onUnexpectedError(error, 'handling call'))
	})
	...
	ws.on('CB:notification', node => {
		processNode('notification', node, 'handling notification', handleNotification)
			.catch(error => onUnexpectedError(error, 'handling notification'))
	})
