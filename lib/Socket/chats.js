...
import processMessage, { unwrapSecretEncryptedMessage } from '../Utils/process-message.js'
...
export const makeChatsSocket = config => {
	...
	/** this mutex ensures that messages are processed in order per sender */
	const messageMutex = makeKeyedMutex()
	/** this mutex ensures that receipts are processed in order per chat */
	const receiptMutex = makeKeyedMutex()
	/** this mutex ensures that app state patches are processed in order */
	const appStatePatchMutex = makeMutex()
	/** this mutex ensures that notifications are processed in order per chat */
	...
	const buildProfilePictureQueryContent = (type, query, tcTokenContent) => {
		const picture = {
			tag: 'picture',
			attrs: {
				type,
	 ...
	 */
	const profilePictureUrl = async (jid, type = 'preview', content = 'url') => {
		...
		const result = await query(
			{
				...
				content: buildProfilePictureQueryContent(type, content, tcTokenContent)
			},
			30000
		)
