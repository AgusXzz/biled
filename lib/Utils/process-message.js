...
/**
 * Decrypt an encrypted message edit
 * @param edit encrypted edit envelope
 * @param ctx decryption context (original message key, editor JID, encryption key)
 * @returns the inner protocol message containing the edited content
 */
export function decryptMessageEdit(
	{ encPayload, encIv },
	{ origMsgId, origMsgSenderJid, editorJid, msgEncKey }
) {
...
/**
 * Unwrap a secretEncryptedMessage into its inner protocol message.
 * Modifies the message IN PLACE: replaces `secretEncryptedMessage` with the
 * decoded `protocolMessage` (MESSAGE_EDIT).
 * @param message the incoming message (mutated on success)
...
export const unwrapSecretEncryptedMessage = async (message, { creds, getMessage, logger }) => {
	...
	try {
		...
		let msgEncKey = origMsg?.message?.messageContextInfo?.messageSecret
			|| origMsg?.messageContextInfo?.messageSecret
		if (typeof msgEncKey === 'string') {
			msgEncKey = Buffer.from(msgEncKey, 'base64')
		}
			...
			case proto.Message.ProtocolMessage.Type.HISTORY_SYNC_NOTIFICATION:
				...
				if (process) {
					...
					if (histNotification.syncType !== proto.HistorySync.HistorySyncType.ON_DEMAND) {
						...
						const prev = creds.processedHistoryMessages || []
						const next = [...prev, { key: message.key, messageTimestamp: message.messageTimestamp }]
						ev.emit('creds.update', {
							processedHistoryMessages: next.length > MAX_PROCESSED_HISTORY
								? next.slice(next.length - MAX_PROCESSED_HISTORY)
								: next
						})
					}
