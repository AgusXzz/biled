...
import { version as baileysVersion } from '../Defaults/index.js'
...
const STREAM_BUFFER_MAX_SIZE = 50 * 1024 * 1024
export const streamToBuffer = async stream => {
	let totalLen = 0
	const chunks = []
	for await (const chunk of stream) {
		totalLen += chunk.length
		if (totalLen > STREAM_BUFFER_MAX_SIZE) {
			throw new Error(`streamToBuffer: exceeded max size of ${STREAM_BUFFER_MAX_SIZE} bytes`)
		}
		chunks.push(chunk)
	}
	return Buffer.concat(chunks)
}
export const safeBufferFrom = data => {
	if (data === null || data === undefined) return Buffer.alloc(0)
	return Buffer.isBuffer(data) ? data : Buffer.from(data)
}
const iqIdCounter = { current: 0 }
export const makeIqId = () => {
	iqIdCounter.current = (iqIdCounter.current + 1) % 1000000
	return `iq_${iqIdCounter.current}_${Date.now()}`
}
export const makeIqNode = (type, xmlns, content, extraAttrs) => ({
	tag: 'iq',
	attrs: {
		id: makeIqId(),
		type,
